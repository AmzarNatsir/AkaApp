        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-header">
                    <h4>Data Merek</h4><span>Daftar data merek</span>
                </div>
                <div class="card-body">
...
<script>
    $(document).ready(function () {
        ...
        var tableAjax = $("#table_view").DataTable({
            ajax: "{{ route('datamaster.merek.getdata') }}",
            processing: true,
            serverSide: true,
            autoWidth: true,
            columns: [
        ...
        $("#btn_add").on("click", function(){
            $("#form_view").load("{{ route('datamaster.merek.create') }}");
        });
        $("#table_view").on('click', '#btn_edit', function(){
            $("#form_view").load("{{ url('dataMaster/merek/edit') }}/"+$(this).val());
        });
    });
    var konfirmDelete = function(el)
    {
        ...
        {
            $.ajax({
                url: "{{ url('dataMaster/merek/destroy') }}/"+$(el).val(),
                type: "GET",
